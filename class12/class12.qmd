...
Here we will use the DESeq2 package for RNASeq analysis. The data for todays class comes from a srudy of airway smooth muscle cells treated with dexamethasone,  a synthetic glucocoticoidsteriod with anti-inflammatory effects (Himes et al. 2014).
...
(a) We need to find which columns in countData are "control" samples.
  - lookin the metadata (a.k.a. colData), specifically the $dex column

```{r}
control.inds <- metadata$dex == "control"
```

(b) Extract all the control columns from `colData` and call it `control.counts`
...
if we have no change - i.e. some values in control and treated we will have log2 fold-change.
...
It is the known P-values that we care about and these are lost in the skewed plot above. Let's take the log of the $padj values for our plot.
...
We can translate (a.k.a "map") because all these database id formats:
...
res$genename <- mapIds(org.Hs.eg.db,
                     keys=row.names(res), # Our genenames
                     keytype="ENSEMBL",   # The fomrat of our genenames     
